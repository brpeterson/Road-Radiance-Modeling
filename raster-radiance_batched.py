...
def main():
	...
	road_chunks = np.array_split(road_subset, num_processes*750)
	...
	batch_count = math.ceil(len(road_chunks) / 450)
	...
	try:
		for i, batch in enumerate(chunk_batches):
			with multiprocessing.Pool(processes=num_processes) as pool:
				batch_results = pool.map(calculate_impact, [(chunk, sample_pixels, lookup_table) for chunk in batch])
